...
namespace GPUInstancerPro.TerrainModule
{
    public abstract class GPUITerrainManager<T> : GPUIManagerWithPrototypeData<T> where T : GPUIPrototypeData, new()
    {
        ...
        [NonSerialized]
        protected bool _hasPendingTerrainTransformChanges;
        [NonSerialized]
        protected int _pendingTerrainTransformChangesFrame;
        ...
        public void RequireTerrainTransformChange(int frameCount)
        {
            if (_hasPendingTerrainTransformChanges) // If called second time before applied, require update.
            {
                if (_pendingTerrainTransformChangesFrame == frameCount)
                    return;

                if (this is GPUIDetailManager detailManager)
                    detailManager.RequireUpdate(true);
                else
                    RequireUpdate();
                return;
            }
            _hasPendingTerrainTransformChanges = true;
            _pendingTerrainTransformChangesFrame = frameCount;
        }

        protected void CheckPendingTerrainTransformChanges()
        {
            if (_isTerrainsModified)
                ApplyTerrainModifications();

            if (!_hasPendingTerrainTransformChanges)
                return;

            Profiler.BeginSample("GPUITerrainManager.CheckPendingTerrainTransformChanges");
            Dictionary<int, GPUITerrain>.ValueCollection activeTerrains = GetActiveTerrainValues();
            if (activeTerrains.Count > 0)
            {
                int counter = 0;
                Matrix4x4 deltaLTW = GPUIConstants.IDENTITY_Matrix4x4;
                bool isUniform = true;
#if UNITY_EDITOR
                if (!Application.isPlaying)
                    isUniform = false;
                    ...
                    {
                        if (counter == 0)
                            deltaLTW = gpuiTerrain.deltaLTW;
                        else if (!deltaLTW.Approximately(gpuiTerrain.deltaLTW))
                        {
                            isUniform = false;
                            break;
                        }
                ...
                {
                    GPUITransformBufferUtility.ApplyMatrixOffsetToTransforms(this, deltaLTW); // Rotate and move with compute shader.
                }
            }
        ...
        public bool HasNullOrDuplicateTerrains()
        {
            return GPUIUtility.HasNullOrDuplicate(_gpuiTerrains);
        }

        public void RemoveNullOrDuplicateTerrains()
        {
            GPUIUtility.RemoveNullsAndDuplicates(_gpuiTerrains);
#if UNITY_EDITOR
            if (!Application.isPlaying)
        ...
        public bool AddTerrain(Terrain terrain)
        {
            ...
            GPUITerrain gpuiTerrain = terrain.GetComponent<GPUITerrain>();
            if (gpuiTerrain == null)
                gpuiTerrain = terrain.AddOrGetComponent<GPUITerrainBuiltin>(allowUndo: true);
            return AddTerrain(gpuiTerrain);
        }
        ...
        public bool ContainsTerrains(IEnumerable<Terrain> terrains)
        {
            ...
            foreach (var terrain in terrains)
            {
                if (!ContainsTerrain(terrain) && (!terrain.TryGetComponent(out GPUITerrain gpuiTerrain) || gpuiTerrain is GPUITerrainBuiltin))
                    return false;
            }
            return true;
        }
