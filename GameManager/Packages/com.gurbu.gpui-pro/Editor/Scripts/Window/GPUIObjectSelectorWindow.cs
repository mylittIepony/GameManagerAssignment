...
namespace GPUInstancerPro
{
    public class GPUIObjectSelectorWindow : GPUIEditorWindow
    {
        ...
        private Type _componentTypeFilter;
        ...
        //{
        //    ShowWindow("t:prefab", true, false, false, true, null, typeof(GPUIPrefabBase));
        //}

        public static GPUIObjectSelectorWindow ShowWindow(string filter, bool showInstanceCounts, bool allowModelPrefabs, bool allowMultiSelect, bool allowSkinnedMeshRenderer, UnityAction<List<UnityEngine.Object>> prefabSelectionAction, Type componentTypeFilter = null)
        {
            ...
            window._componentTypeFilter = componentTypeFilter;
            window.Initialize();
        ...
        private List<UnityEngine.Object> FindObjects()
        {
            ...
            bool isTexture2D = _filter.ToLower().Contains("t:texture2d");
            string loadingText = "Loading Prefabs";
            if (!isPrefab)
                loadingText = "Loading Objects";

            EditorUtility.DisplayProgressBar(loadingText, loadingText + "...", 0f);
            ...
            bool hasComponentTypeFilter = _componentTypeFilter != null;

            int guidCount = guids.Length;
            for (int i = 0; i < guidCount; i++)
            {
                if (i % 11 == 10)
                    EditorUtility.DisplayProgressBar(loadingText, loadingText + "...", (float)i / guidCount);
                string path = AssetDatabase.GUIDToAssetPath(guids[i]);
                if (isPrefab)
                {
                    if (!TryGetPrefab(path, out GameObject prefab))
                        continue;
                    if (hasComponentTypeFilter && prefab.GetComponent(_componentTypeFilter) == null)
                        continue;
        ...
        public bool TryGetPrefab(string path, out GameObject prefab)
        {
            ...
            bool isModelPrefab = ext == ".fbx" || ext == ".obj" || ext == ".dae" ||
                                 ext == ".3ds" || ext == ".blend" || ext == ".skp" ||
                                 ext == ".ma" || ext == ".mb";

            if (!isPrefab && !isModelPrefab)
                return false;

            // Skip container prefabs by size
            if (isPrefab && fi.Length > (1024 * 10000))
                return false;

            string contents = File.ReadAllText(path);

            // Normal prefab check from file content
            if (isPrefab)
            {
                bool hasMesh = contents.Contains("MeshRenderer");
                bool hasSkinned = _allowSkinnedMeshRenderer && contents.Contains("SkinnedMeshRenderer");
                bool isVariant = contents.Contains("PrefabInstance");

                if (!hasMesh && !hasSkinned && !isVariant)
                    return false;
                ...
                if (isVariant)
                {
                    if (prefab.GetComponentInChildren<MeshRenderer>(true) == null &&
                        (!_allowSkinnedMeshRenderer || prefab.GetComponentInChildren<SkinnedMeshRenderer>(true) == null))
                        return false;
                }
            }
            else // Model prefab
            ...
            while (stack.Count > 0)
            {
                ...
                // If this transform has GPUIPrefabDefinition, skip its entire subtree
                if (t.GetComponent<GPUIPrefabDefinition>() != null)
                    continue;

                // Count renderers (skinned or normal)
                MeshRenderer mr = t.GetComponent<MeshRenderer>();
                if (mr != null)
                {
                    rendererCount++;
                    if (rendererCount > maxRendererCount)
                        return false; // Too many renderers
                }

                if (_allowSkinnedMeshRenderer)
                {
                ...
                // Check for LODGroup on children only (not on root)
                if (t != root)
                {
