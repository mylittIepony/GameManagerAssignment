...
namespace GPUInstancerPro
{
    ...
    public class GPUIShaderInstancePropertyDrawer : PropertyDrawer
    {
        ...
        private void DrawPrototypeFields(SerializedProperty property)
        {
            if (property == null) 
                return;
            SerializedProperty shaderNameSP = null;
            try
            {
                shaderNameSP = property.FindPropertyRelative("shaderName");
            }
            catch (System.ObjectDisposedException)
            {
                return;
            }
            if (shaderNameSP == null)
                return;
            string originalShaderName = shaderNameSP.stringValue;
