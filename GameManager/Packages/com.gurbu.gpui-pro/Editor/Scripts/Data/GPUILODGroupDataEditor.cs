...
namespace GPUInstancerPro
{
    ...
    public class GPUILODGroupDataEditor : GPUIEditor
    {
...
#if GPUIPRO_DEVMODE
            Button generateBoundsButton = new Button(() =>
            {
                Bounds bounds = serializedObject.FindProperty("bounds").boundsValue;
                GameObject cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
                cube.name = _lodGroupData.prototype.prefabObject.name + " Bounds Rep.";
                cube.transform.position = bounds.center;
                cube.transform.localScale = bounds.size;
            });
            generateBoundsButton.text = "Generate Bounds Representation";
            contentElement.Add(generateBoundsButton);
#endif
        ...
        private void OnSliderMove(MouseMoveEvent mouseMoveEvent)
        {
            ...
            //Debug.Log(GPUIConstants.LOG_PREFIX + "sliderPercentage: " + sliderPercentage + " ToTransitionValue: " + (/*1f -*/ ToTransitionValue(sliderPercentage)));
