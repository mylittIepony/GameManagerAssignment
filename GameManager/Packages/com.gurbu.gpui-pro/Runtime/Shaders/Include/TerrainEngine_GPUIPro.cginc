...
float4 _GPUIWindZone;
float3 _GPUIWindDirection;
// Detail bending
...
#if defined(GPUI_PRO_ACTIVE)
    #if defined(GPUI_TREE_CREATOR_WIND)
        // Position-based main wind phase
        float spatialPhase = dot(unity_ObjectToWorld._14_24_34, _GPUIWindDirection) * 0.2; // wave size
        float t = (_Time.y * 12.0 * _GPUIWindZone.z) + spatialPhase;

        // Small per-instance variation
        float instanceHash = frac(sin((gpui_InstanceID + 1) * 12.9898) * 43758.5453);
        float phaseOffset = instanceHash * 0.5; // small offset (~0.5 radians)

        // Base sway + irregular component
        float sway = sin(t * 0.15 + phaseOffset);
        float irregular = cos(t * 0.23 + phaseOffset * 2.17);

        // Blend into a final strength
        float bendStrength = (sway * 0.65 + irregular * 0.35 + 0.7);

        // Rotate wind into object space
        float3 windDirObject = mul((float3x3)unity_WorldToObject, _GPUIWindDirection * _GPUIWindZone.w * 0.7);

        // Branch shake
        pos.xyz += ((vWavesSum.xyx * (animParams.y * fDetailAmp * normal.xyz)) +
                    (windDirObject * vWavesSum.y * animParams.w))
                    * _GPUIWindZone.x * _GPUIWindZone.z * bendStrength;

        // Primary bending
        pos.xyz += animParams.z * windDirObject * _GPUIWindZone.y * bendStrength;
    #else
        // Rotate world wind into object space per instance
        float3 windDirObject = mul((float3x3) unity_WorldToObject, _Wind.xyz);
        pos.xyz += ((vWavesSum.xyx * bend) + (windDirObject * vWavesSum.y * animParams.w)) * _Wind.w;
        pos.xyz += animParams.z * windDirObject;
    #endif // GPUI_TREE_CREATOR_WIND
