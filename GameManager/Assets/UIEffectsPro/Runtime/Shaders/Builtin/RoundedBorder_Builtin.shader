// UIEffectsPro/Runtime/Shaders/Builtin/RoundedBorder_Builtin.shader 
// Version 3.1 - Built-in Pipeline amb Background Blur (GrabPass) 
Shader "UIEffects/RoundedBorder_Builtin"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        // GrabPass for Background Blur (ONLY for Built-in Pipeline)
        GrabPass 
        {
        ...
        Pass
        {
            ...
            // --- MODIFICACIÓ B (Built-in) ---
            fixed4 _ProgressColorStart;
            fixed4 _ProgressColorEnd;
            float _UseProgressColorGradient;
            // --- FI MODIFICACIÓ B (Built-in) ---
            ...
            fixed4 blendTexture(fixed4 base, fixed4 overlay, float blendMode)
            {
                fixed4 result = base;
            ...
            fixed4 applyBoxBlur(sampler2D sourceTex, float4 sourceTexelSize, float2 uv, float radius, int iterations)
            {
                if (radius < 0.1 || iterations < 1)
                {
                    return tex2D(sourceTex, uv);
                }
                
                iterations = clamp(iterations, 1, 8);
                fixed4 result = fixed4(0, 0, 0, 0);
                float totalWeight = 0.0;
                float2 baseTexelSize = sourceTexelSize.xy;
                ...
                offsets[5]  = float3(-2, -1, 0.015019); offsets[6]  = float3(-1, -1, 0.059912); offsets[7]  = float3( 0, -1, 0.094907);
                offsets[8]  = float3( 1, -1, 0.059912); offsets[9]  = float3( 2, -1, 0.015019);
                offsets[10] = float3(-2,  0, 0.023792); offsets[11] = float3(-1,  0, 0.094907); offsets[12] = float3( 0,  0, 0.150342);
                offsets[13] = float3( 1,  0, 0.094907); offsets[14] = float3( 2,  0, 0.023792);
                offsets[15] = float3(-2,  1, 0.015019); offsets[16] = float3(-1,  1, 0.059912); offsets[17] = float3( 0,  1, 0.094907);
                offsets[18] = float3( 1,  1, 0.059912); offsets[19] = float3( 2,  1, 0.015019);
                offsets[20] = float3(-2,  2, 0.003765); offsets[21] = float3(-1,  2, 0.015019); offsets[22] = float3( 0,  2, 0.023792);
                offsets[23] = float3( 1,  2, 0.015019); offsets[24] = float3( 2,  2, 0.003765);
                ...
                for (int iter = 1; iter <= 8; iter++)
                {
                    ...
                    for (int i = 0; i < 25; i++)
                    {
                        float2 sampleUV = uv + offsets[i].xy * texelSize;
                        fixed4 sample = tex2D(sourceTex, sampleUV);
            ...
            // BACKGROUND BLUR (Built-in Pipeline - usa GrabPass)
            fixed4 applyBackgroundBlur(float2 screenUV, float radius, int iterations, int downsample)
            {
                if (radius < 0.1 || iterations < 1)
                {
                ...
                fixed4 result = fixed4(0, 0, 0, 0);
                float totalWeight = 0.0;
                float2 baseTexelSize = _BackgroundTexture_TexelSize.xy;
                ...
                for (int iter = 1; iter <= 8; iter++)
                {
                    ...
                    for (int i = 0; i < 25; i++)
                    {
                        ...
                        fixed4 sample = tex2D(_BackgroundTexture, sampleUV);
                        float weight = offsets[i].z * iterWeight;
            ...
            float sampleSpriteAlphaWithBlur_AtlasSafe(float2 baseUV, float blurRadius)
            {
                if (blurRadius < 0.1)
                {
                    ...
                    return tex2D(_MainTex, baseUV).a;
                }
                ...
                for (int y = -5; y <= 5; y++)
                {
                    ...
                    for (int x = -5; x <= 5; x++)
                    {
                        ...
                        float alpha = tex2D(_MainTex, sampleUV).a * isInside;
            ...
            float getSpriteBorderMask(float2 uv, float width)
            {
                ...
                float n  = tex2D(_MainTex, uv + float2( 0,  1) * texelSize).a;
                float s  = tex2D(_MainTex, uv + float2( 0, -1) * texelSize).a;
                float e  = tex2D(_MainTex, uv + float2( 1,  0) * texelSize).a;
                float w  = tex2D(_MainTex, uv + float2(-1,  0) * texelSize).a;
                float ne = tex2D(_MainTex, uv + float2( 1,  1) * texelSize).a;
                float nw = tex2D(_MainTex, uv + float2(-1,  1) * texelSize).a;
                float se = tex2D(_MainTex, uv + float2( 1, -1) * texelSize).a;
                float sw = tex2D(_MainTex, uv + float2(-1, -1) * texelSize).a;
                ...
                if (width > 1.0)
                {
                    ...
                    float n2 = tex2D(_MainTex, uv + float2(0, 1) * wideTexelSize).a;
                    float s2 = tex2D(_MainTex, uv + float2(0, -1) * wideTexelSize).a;
                    float e2 = tex2D(_MainTex, uv + float2(1, 0) * wideTexelSize).a;
                    float w2 = tex2D(_MainTex, uv + float2(-1, 0) * wideTexelSize).a;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                fixed4 originalTexture = tex2D(_MainTex, textureUV);
                ...
                if (mainAlpha > 0.001 && spriteAlpha > 0.01)
                {
                    ...
                    // Background Blur (Built-in Pipeline)
                    ...
                    if (applyNormalFlow)
                    {
                        ...
                        if (_EnableBlur > 0.5 && _BlurType < 0.5) 
                        {
                            ...
                            contentColor = applyBoxBlur(_MainTex, _MainTex_TexelSize, textureUV, blurRadius, iterations);
                        }
                        else
                        {
                        ...
                        if (_EnableTexture > 0.5)
                        {
                            ...
                            if (_EnableBlur > 0.5 && _BlurType < 0.5)
                            {
                                int iterations = (int)_BlurIterations;
                                float blurRadius = _BlurRadius / max(_BlurDownsample, 1.0);
                                textureColor = applyBoxBlur(_OverlayTexture, _OverlayTexture_TexelSize, overlayUV, blurRadius, iterations);
                            }
                            else
                            {
                                textureColor = tex2D(_OverlayTexture, overlayUV);
                            }

                            if (textureColor.a > 0.001)
                            {
                                fixed4 originalColor = contentColor;
                                fixed4 fullEffectColor = blendTexture(originalColor, textureColor, _TextureBlendMode);
                    ...
                    fixed4 shapeColor = contentColor;
                    ...
                    if (_BorderWidth > 0.0)
                    {
                        ...
                        fixed4 finalBorderColor = _BorderColor;
