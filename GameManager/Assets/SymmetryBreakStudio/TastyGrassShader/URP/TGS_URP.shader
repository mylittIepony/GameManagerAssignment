Shader "Hidden/Symmetry Break Studio/Tasty Grass Shader/URP"
{
    ...
    SubShader
    {
        ...
        InputData TgsGetUrpInputData(TgsFragmentInput input)
        {
            ...
            //NOTE: shadow coord is always computed in frag.
            #endif
        ...
        TgsFragmentInput tgsCommonVertexShader(Attributes input, uint vertexID: SV_VertexID)
        {
            ...
            float3 positionPivotWs;
            TgsGetGrassVertex(vertexID, _Time.x, 1.0, /*out*/ positionWS, /*out*/ positionPivotWs, /*out*/
                                           packedNormalColor, /*out*/ barycentricCoord);
        ...
        half4 frag_depth_normal(TgsFragmentInput input) : SV_Target
        {
            ...
            const SurfaceData urpSurfaceData =
                TgsGetUrpSurfaceData(input.barycentricCoord, input.bin_normalXY_colorRGB);
            return TgsPreReturn(float4(urpInputData.normalWS, urpSurfaceData.alpha));
        }
        ...
        GBufferFragOutput frag_gbuffer(TgsFragmentInput input)
        {
        ...
        FragmentOutput frag_gbuffer(TgsFragmentInput input)
        {
            ...
            InitializeBRDFData(urpSurfaceData.albedo, urpSurfaceData.metallic, urpSurfaceData.specular,
                urpSurfaceData.smoothness, urpSurfaceData.alpha, brdfData);
            Light mainLight = GetMainLight(inputData.shadowCoord, inputData.positionWS, inputData.shadowMask);


            MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI, inputData.shadowMask);
            half3 color = GlobalIllumination(brdfData, inputData.bakedGI, urpSurfaceData.occlusion,
               inputData.positionWS, inputData.normalWS, inputData.viewDirectionWS);
            #if UNITY_VERSION >= 60020000
            return PackGBuffersBRDFData(brdfData, inputData, urpSurfaceData.smoothness, urpSurfaceData.emission + color,
urpSurfaceData.occlusion);
            #else
            return BRDFDataToGbuffer(brdfData, inputData, urpSurfaceData.smoothness, urpSurfaceData.emission + color,
                                     urpSurfaceData.occlusion);
            #endif
        }
        ENDHLSL
