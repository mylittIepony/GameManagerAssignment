...
float TgsLengthSqr(float3 vec)
{
    return dot(vec, vec);
}

bool TgsAABBTest(float3 p, float3 min, float3 max)
{
    // TODO: is this still good? hows performance?
    return
        min.x <= p.x && p.x < max.x &&
        min.y <= p.y && p.y < max.y &&
        min.z <= p.z && p.z < max.z;
    ...
    // Minimal branching AABB test.
    const float3 p_clamped = clamp(p, min, max);
    // NOTE: a bitwise comparison is fine here, since we don't expect the value to change if inside the AABB.
    // return p_clamped == p;
    return TgsLengthSqr(p - p_clamped) <= 0.05;
}
...
float3 TgsApplyWind(float3 pivotWs, float3 positionWS, float3 normalWS, float time, float windScale)
{
    float3 pivot = pivotWs + normalWS * _WindPivotOffsetByHeight;
    const float bladeLength = windScale;
    ...
    float windIntensity = snoise(positionWS.xyz * windPatchSize + time * windSpeed * -windRotationAxis.zyx) *
        windStrength * bladeLength;
    ...
    {
        output.positionWS.xyz = verticesWs[vertexID % 3];
        //UnpackUnitToFloat3(vertices[vertexID % 3], _PositionBoundMin, _PositionBoundMax).xyz;
    }
    #endif
    return positionWS;
}
...
void TgsGetGrassVertex(uint vertexID, float time, float heightScale, out float3 positionWS, out float3 positionPivotWS,
                       out uint normalColorBinRaw, out float3 barycentricCoord)
{
    ...
    // Apply Wind
    // =====================================================================================================
    const float bladeLength = distance(positionPivotWS, positionWS);
    positionWS = TgsApplyWind(positionPivotWS, positionWS, normalWS, time, bladeLength);
...
float SharpenPixelPerfect(float alpha)
{
    ...
    const float alphaDivClamped = max(alphaAbsSize, HALF_MIN);
...
float2 TgsGetGrassUv(float3 barycentricCoord)
{
    const float2 uvUpperRightTriangle = float2(barycentricCoord.z, 1.0 - barycentricCoord.x);
    const float2 uvFromQuad = float2(uvUpperRightTriangle.x + 0.5 * (1.0 - uvUpperRightTriangle.y),
                                     uvUpperRightTriangle.y);
    return lerp(uvUpperRightTriangle, uvFromQuad, _UvUseCenterTriangle);
}

void TgsGetSurfaceParametersInner(float3 barycentricCoord, float3 bladeColor, out float3 albedo, out float alpha,
                                  out float smoothness, out float occlusion)
{
    float2 uv = TgsGetGrassUv(barycentricCoord);
...
void TgsGetSurfaceParameters(float3 barycentricCoord, int packedNormalAndColor, out float3 albedo, out float alpha,
                             out float smoothness, out float occlusion)
{
    float3 bladeColor = TgsUnpackColor(packedNormalAndColor);
    TgsGetSurfaceParametersInner(barycentricCoord, bladeColor, /*out*/ albedo, /*out*/ alpha, /*out*/ smoothness,
                                 /*out*/ occlusion);
}
...
void TgsGetGrassVertexShaderGraph_float(float vertexID, float time, float heightScale, out float3 positionWS,
                                        out float3 positionPivotWS, out float3 normal, out float3 color,
                                        out float3 barycentricCoord)
{
    uint normalColorBinRaw;
    TgsGetGrassVertex((uint)vertexID, time, heightScale, /*out*/ positionWS, /*out*/ positionPivotWS, /*out*/
                      normalColorBinRaw, /*out*/ barycentricCoord);
    color = TgsUnpackColor(normalColorBinRaw);
    normal = TgsUnpackNormal(normalColorBinRaw);
}

void TgsGetSurfaceParametersShaderGraph_float(float3 barycentricCoord, float3 color, out float3 albedo, out float alpha,
                                              out float smoothness, out float occlusion)
{
    TgsGetSurfaceParametersInner(barycentricCoord, color, /*out*/ albedo, /*out*/ alpha, /*out*/ smoothness, /*out*/
                                 occlusion);
}

void TgsGetGrassUv_float(float3 barycentricCoord, out float2 uv)
{
    uv = TgsGetGrassUv(barycentricCoord);
}

void TgsAlphaClipAndClear_float(float alpha, out float outAlpha)
{
    clip(alpha);
    outAlpha = 1.0; // Set the alpha to 1.0, so that in XR-Passthrough, no "leaks" to the background appear.
}
#endif
