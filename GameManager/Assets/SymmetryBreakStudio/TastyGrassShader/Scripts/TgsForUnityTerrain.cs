...
namespace SymmetryBreakStudio.TastyGrassShader
{
    ...
    [HelpURL(
        "https://github.com/SymmetryBreakStudio/TastyGrassShader/wiki/1.-Quick-Start#adding-grass-to-a-terrain--unity-terrain")]
    ...
    public class TgsForUnityTerrain : MonoBehaviour
    {
        ...
        private Bounds lastBrushBounds;
#endif
        ...
        void OnDrawGizmosSelected()
        {
            if (showChunksBounds)
            {
                ...
                Gizmos.DrawWireCube(TgsTerrainLayer.lastBrushBounds.center, TgsTerrainLayer.lastBrushBounds.size);
#endif
            }
        }
        ...
        public SharedTools.TgsStats GetMemoryStats()
        {
            ...
            foreach (TgsTerrainLayer layer in layers)
            {
    ...
    public class TgsTerrainLayer
    {
        ...
        [FormerlySerializedAs("densityMode")] [Tooltip("What mode is used to control the amount of the layer.")]
        ...
        #region Painting Storage

        [FormerlySerializedAs("paintTexture")] [FormerlySerializedAs("storageScene")] [ResizableTexture2D]
...
#if TASTY_GRASS_SHADER_DEBUG
        public static Bounds lastBrushBounds;
#endif
        ...
        void CreatePainStorageTexture()
        {
            const int defaultResolution = 128;
            paintedDensityMapStorage = new Texture2D(defaultResolution, defaultResolution,
                TextureFormat.R8, false)
            {
                wrapMode = TextureWrapMode.Clamp,
                name = "Tgs Paint Storage"
            };

            // fill with black
            NativeArray<Color> color =
                new NativeArray<Color>(paintedDensityMapStorage.width * paintedDensityMapStorage.height,
                    Allocator.Temp);
            paintedDensityMapStorage.SetPixelData(color, 0);
            color.Dispose();
        ...
        {
            Profiler.BeginSample("CheckForSettingsChange");
            Hash128 newHash = new();
            ...
            List<RectInt> changedTexelRegions, int unityLayerMask)
        {
            ...
            Vector2Int chunkSizePixels = new(
                (int)TgsInstance.CeilingDivisionFloat(TgsGlobalSettings.GlobalChunkSize, pixelSize.x),
                (int)TgsInstance.CeilingDivisionFloat(TgsGlobalSettings.GlobalChunkSize, pixelSize.z));
            ...
            if (distribution == TerrainLayerDistribution.TastyGrassShaderPaintTool)
            {
                ...
                {
                    CreatePainStorageTexture();
                }

                if (_paintedDensityMapRt == null || _paintedDensityMapRt.width != paintedDensityMapStorage.width ||
                    _paintedDensityMapRt.height != paintedDensityMapStorage.height)
                {
                    ...
                    _paintedDensityMapRt = new RenderTexture(paintedDensityMapStorage.width,
                        paintedDensityMapStorage.height,
            ...
            SharedTools.SetupChunks(_chunks, chunksTotal);
            ...
            if (_drawPending)
            {
                ...
                paintBrushDiameterInPixelSpace = _drawRadius * (_paintedDensityMapRt.width / terrainSizeXZ.x) * 2.0f;
...
#if TASTY_GRASS_SHADER_DEBUG
                lastBrushBounds = paintBrushBoundsWs;
#endif
            }
            ...
            switch (distribution)
            {
                ...
                case TerrainLayerDistribution.TastyGrassShaderPaintTool:
                    ...
                    if (paintedDensityMapDirty || paintedDensityMapStorage == null)
                    {
                        ...
                        SharedTools.StoreRenderTexture(_paintedDensityMapRt, paintedDensityMapStorage);
                    }
            ...
            for (int chunkIndex = 0; chunkIndex < chunksTotal; chunkIndex++)
            {
                ...
                Vector2Int chunkXy = new(chunkIndex / chunksPerAxis.x, chunkIndex % chunksPerAxis.x);
                CalculateHeightmapChunkBounds(chunkXy, chunkSizePixels, terrainHeightmapRes, pixelSize, terrainData,
                    terrainFullBounds, out Vector2Int pixelBoundsMin, out Bounds chunkBounds,
                    out Vector2Int pixelBoundsSize);
...
// #endif
        ...
        public static void CalculateHeightmapChunkBounds(Vector2Int chunkXy, Vector2Int chunkSizePixels,
            Vector2Int terrainHeightmapResolution, Vector3 pixelSizeToMeters, TerrainData terrainData,
            Bounds terrainFullBounds,
            out Vector2Int pixelBoundsMin, out Bounds chunkBounds, out Vector2Int pixelBoundsSize)
        {
            pixelBoundsMin = chunkXy * chunkSizePixels;
            Vector2Int pixelBoundsMax = (chunkXy + Vector2Int.one) * chunkSizePixels;
            RectInt pixelRect = new();
            pixelRect.SetMinMax(pixelBoundsMin, pixelBoundsMax);
            pixelBoundsMax = Vector2Int.Min(terrainHeightmapResolution - Vector2Int.one, pixelBoundsMax);

            Vector3 chunkStart = new(pixelBoundsMin.x * pixelSizeToMeters.x, 0.0f,
                pixelBoundsMin.y * pixelSizeToMeters.z);
            Vector3 chunkEnd = new(pixelBoundsMax.x * pixelSizeToMeters.x, terrainData.size.y,
                pixelBoundsMax.y * pixelSizeToMeters.z);
            ...
            pixelBoundsSize = pixelBoundsMax - pixelBoundsMin;
        }
        ...
        public void Release()
        {
            ...
            SharedTools.SetupChunks(_chunks, -1);
        }
        ...
        public void DrawGizmos()
        {
            ...
            SharedTools.DrawChunksGizmos(_chunks);
        }
